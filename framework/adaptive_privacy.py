...
사용법: 선택된 클라이언트들의 평균 참여율로 라운드별 예산 계산
- 서버 집계 후 한 번만 노이즈 추가 (표준 DP-FedAvg)
- 평균 ε_t를 사용하여 전체 집계 gradient에 노이즈

직관:
- 자주 참여하는 클라이언트 (높은 p_i) → 작은 ε_i → 강한 프라이버시 보호
- 간헐적 참여 클라이언트 (낮은 p_i) → 큰 ε_i → 높은 유틸리티
"""
...
class AdaptivePrivacyAllocator:
    ...
    def compute_privacy_budget(self, participation_rate: float) -> float:
        ...
        Args:
            participation_rate: 참여율 (0.0 ~ 1.0)
                일반적으로 선택된 클라이언트들의 평균 참여율
