...
Non-IID 데이터 분포를 가진 MNIST 및 CIFAR-10 데이터셋에 대한 광범위한 실험 결과, QuAP-FL은 MNIST에서 93.30\%의 정확도를 달성하였다. 이는 프라이버시 보호가 없는 FedAvg(93.26\%)와 대등하며, 고정형 DP 방식(Fixed-DP, 93.76\%) 대비 0.46\%p 낮지만, QuAP-FL은 \textbf{클라이언트별 참여 이력 추적, 분위수 기반 클리핑, 적응형 예산 할당} 등 추가적인 메커니즘을 수행하면서도 이러한 미미한 성능 저하에 그쳤다는 점에서 의의가 있다. 특히 CIFAR-10에서는 QuAP-FL(76.82\%)이 Fixed-DP(75.80\%) 대비 1.02\%p 높은 성능을 보여, 복잡한 데이터셋일수록 적응형 메커니즘의 이점이 부각됨을 확인하였다. 본 연구는 적절한 수준의 DP 노이즈가 모델의 과적합(Overfitting)을 방지하는 정규화(Regularization) 효과를 제공하여, Non-IID 환경에서 오히려 일반화 성능을 높일 수 있음을 실험적으로 규명하였다. 본 연구는 참여 패턴이 불균형한 현실적인 연합학습 환경에서 프라이버시 보장과 모델 유틸리티 사이의 최적 균형점을 찾는 실용적이고 효과적인 프레임워크를 제시한다.
\end{abstract}
...
따라서 QuAP-FL은 실용성과 프라이버시 간의 타협안으로서, \textbf{마지막 분류 레이어(Critical Layer)에만 집중적으로 노이즈를 주입}하는 전략을 취한다.

\textbf{이론적 근거}: 분류 레이어는 학습 데이터의 클래스 정보를 직접적으로 인코딩하므로, 역전파 공격에 가장 취약하다. 반면, 앞단의 합성곱 층은 일반적인 특징을 학습하며, 이는 데이터셋 간 전이 학습(Transfer Learning)이 가능하다는 점에서 데이터 의존성이 상대적으로 낮다.

\textbf{실용적 고려}: 전체 파라미터에 노이즈를 주입하면 $\mathcal{O}(\sqrt{d})$ 규모의 노이즈가 필요하여 유틸리티가 급격히 저하된다. 본 연구는 전체 파라미터의 약 1-2\%에 해당하는 분류 레이어만 보호함으로써, 노이즈 크기를 $\sqrt{0.02d}$로 줄여 성능을 유지한다.

\begin{equation}
    \tilde{g}_{global} = g_{global} + [0, \dots, 0, \mathcal{N}(0, \sigma^2 I_{critical})]
\end{equation}

\textbf{한계 인정}: 이는 엄밀한 의미의 전체 모델 차분 프라이버시를 보장하지 않는다. 그러나 민감한 정보가 집중된 분류기를 보호함으로써, 실용적인 수준의 프라이버시 보호 효과를 제공한다. 엄밀한 전체 모델 DP가 요구되는 경우, 희소화(Sparsification) 등의 기법을 병행해야 한다(향후 연구).
...
우리의 예산 할당 함수 (Equation 4)에 의해 모든 $t$에 대해 다음이 보장된다.
\begin{equation}
    \epsilon_i(t) \le \epsilon_{base}(1+\alpha) = \frac{\epsilon_{total}}{T}(1+\alpha)
\end{equation}

따라서 최악의 경우(Worst-case), 즉 클라이언트가 매 라운드 참여하는 경우($k_i=T$)에도 총 손실은 다음과 같이 제한된다.

\begin{equation}
    \epsilon_{total, i} \le T \cdot \frac{\epsilon_{total}}{T}(1+\alpha) = (1+\alpha)\epsilon_{total}
\end{equation}

본 실험에서는 $\alpha=0.5$를 사용하였으므로, 가장 빈번하게 참여하는 클라이언트의 경우에도 $\epsilon_{total, i} \le 1.5\epsilon_{total}$로 제한된다. 여기서 $\alpha$ 항은 참여율이 낮을 때만 활성화되므로, $p_i \to 1$일 때 $\exp(-\beta p_i) \to 0$이 되어 추가 예산은 거의 사라진다. 반면 가끔 참여하는 클라이언트는 $k_i \ll T$이므로, 라운드당 예산 $\epsilon_i(t)$를 크게 받아도 총합은 $\epsilon_{total}$보다 훨씬 작게 유지된다. 이는 모든 클라이언트가 사전에 정의된 프라이버시 예산 범위 내에서 안전함을 증명한다.

\subsection{유틸리티 상한 분석 (Utility Bound Analysis)}
본 절에서는 QuAP-FL의 수렴성에 대한 정성적(qualitative) 분석을 제시한다. 표준 확률적 경사 하강법(SGD) 이론에 따르면, $L$-smooth하고 $\mu$-strongly convex한 목적 함수에 대해 다음이 성립한다.
...
이 정리는 두 가지 중요한 통찰을 제공한다. 첫째, 라운드 수 $T$가 증가함에 따라 오차는 0으로 수렴한다. 둘째, QuAP-FL은 참여율이 낮은 클라이언트에게 더 작은 노이즈(큰 예산)를 할당하므로, 전체적인 평균 노이즈 분산 $\bar{\sigma}^2$을 효과적으로 낮출 수 있다. 즉, $\bar{\sigma}^2_{QuAP} < \bar{\sigma}^2_{Fixed}$가 성립하여, 동일한 수렴 속도 보장 하에서 더 나은 최종 오차를 달성할 수 있다. 이는 고정된 노이즈를 사용하는 기존 DP-FL 대비 더 효율적인 프라이버시-유틸리티 트레이드오프를 제공하는 이론적 근거가 된다.
...
실험 결과에서 세 가지 중요한 사실을 발견할 수 있다.

첫째, \textbf{DP의 정규화 효과 (Regularization Effect)}이다. MNIST 데이터셋에서 Fixed-DP(93.76\%)는 노이즈가 없는 FedAvg(93.26\%)보다 약 0.5\%p 높은 정확도를 기록했다. 이는 Non-IID 환경에서 각 클라이언트가 자신의 로컬 데이터에 과적합(Overfitting)되는 경향이 강한데, 적절한 수준의 DP 노이즈가 이를 완화하여 오히려 전역 모델의 일반화(Generalization) 성능을 높였기 때문이다.

둘째, \textbf{QuAP-FL의 실용성 (Practicality)}이다. QuAP-FL(93.30\%)은 Fixed-DP보다 0.46\%p 낮지만, 다음을 고려할 때 이는 매우 경쟁력 있는 결과이다.
\begin{itemize}
    \item 클라이언트별 참여 이력을 실시간으로 추적하고 예산을 동적으로 할당
    \item 매 라운드 그래디언트 분포를 분석하여 클리핑 임계값을 조정
    \item 빈번한 참여자에게는 강한 프라이버시 보호(작은 예산), 드문 참여자에게는 높은 유틸리티 기여(큰 예산)를 보장
\end{itemize}
이러한 복잡한 적응형 메커니즘에도 불구하고 성능 저하가 미미하다는 것은, 제안하는 전략의 효율성을 입증한다.

셋째, \textbf{확장성 우위 (Scalability Advantage)}이다. CIFAR-10과 같이 더 복잡한 데이터셋에서는 QuAP-FL(76.82\%)이 Fixed-DP(75.80\%)를 1.02\%p 유의미하게 앞섰다. 이는 데이터 분포가 복잡하고 클라이언트 간 이질성이 클수록, 참여율에 기반한 정교한 예산 조절이 성능 유지에 중요한 역할을 함을 시사한다.
...
분석 결과, \textbf{Layer-wise Noise} 기법이 성능 향상에 가장 크게 기여(+2.70\%)한 것으로 나타났다. 이는 고차원 모델에서 전체 파라미터에 노이즈를 주입할 때 발생하는 차원의 저주 문제가 얼마나 심각한지를 반증한다. 또한 \textbf{Adaptive Budget} 기법(+1.72\%)은 이질적인 참여 환경에서 필수적임을 확인했다. 세 가지 기법을 모두 통합했을 때 최고의 성능(+3.90\%)을 달성했다. 개별 기법의 단순 합(5.57\%)보다는 작으나, 이는 각 기법이 부분적으로 중복된 효과(노이즈 감소)를 목표로 하기 때문이다. 그럼에도 통합 효과가 단일 기법 최대치(+2.70\%) 대비 1.20\%p 추가 향상을 보여, 상호보완적임을 확인하였다.
